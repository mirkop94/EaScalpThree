...
void OnTick()
  {
   ...
   if(OrdersTotal()>0) isTradeOpened = true;
 
   for( i=0 ; i<3 ; i++){
      comparedFirst = iClose(NULL,0,i+1);
      comparedSecond = iClose(NULL,0,i+2);
      if(comparedSecond<comparedFirst){
         prevCandles[i] = true;
      }else {
         prevCandles[i] = false;
      }
   ...
   if ( prevCandles[0] == true && prevCandles[1] == true && prevCandles[2] == true && isTradeOpened == false) {
      
      tradeTicketLong = OrderSend(NULL,OP_BUY,tradeSize,Ask,maxSlippage,Bid-(stopLoss*Point()),Ask+(takeProfit*Point()),NULL,0,clrAzure);
   
   }
      if ( prevCandles[0] == false && prevCandles[1] == false && prevCandles[2] == false && isTradeOpened == false) {
      
      tradeTicketShort = OrderSend(NULL,OP_SELL,tradeSize,Bid,maxSlippage,Ask+(stopLoss*Point()),Bid-(takeProfit*Point()),NULL,0,clrCrimson);
